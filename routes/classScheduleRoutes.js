...
//         // Ambos son futuros, ordenar por fecha más lejana primero
//         return dateB - dateA;
//       } else if (dateA > now) {
//         // Solo 'a' es futuro
//         return -1;
...
//         return 1;
//       } else {
//         // Ambos son pasados, ordenar por fecha pasada más reciente primero
//         return dateA - dateB;
//       }
//     });
    ...
    try {
        ...
        const sortedSchedules = classSchedules.sort((a, b) => {
            ...
            if (dateTimeA > now && dateTimeB > now) {
                // Ambos son futuros, ordenar por fecha más cercana primero
                return dateTimeA - dateTimeB;
            } else if (dateTimeA > now) {
                // Solo 'a' es futuro, ponerlo primero
                return -1;
            } else if (dateTimeB > now) {
                // Solo 'b' es futuro, ponerlo primero
                return 1;
            } else {
                // Ambos ya han pasado, ordenar cronológicamente (la más reciente primero)
                return dateTimeA - dateTimeB;
            }
        });
...
classScheduleRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    mondayDate.setDate(
      filterDate.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1)
    );
    ...
    classSchedules.forEach((schedule) => {
      const existingDay = formattedData.find(
        (item) =>
          item.nombre === schedule.dayWeek && item.fecha === schedule.date
      );

      const instructorName = schedule.instructorInfo
        ? schedule.instructorInfo.instructorName
        : "";
      const instructorId = schedule.instructorInfo
        ? schedule.instructorInfo._id
        : "";
...
classScheduleRouter.put("/:classId", async (req, res) => {
  ...
  try {
    ...
    const isClassScheduleInDb = await ClassSchedule.findByIdAndUpdate(classId, {
      ...req.body,
    });
...
classScheduleRouter.delete("/:classId", async (req, res) => {
  ...
  try {
    ...
    if (!isClassScheduleInDb) {
      return res
        .status(404)
        .json({ message: "Horario de Clase no existe en base de datos" });
    }
...
classScheduleRouter.patch("/:classId", isAuth, async (req, res) => {
  ...
  try {
    ...
    /* 2.
         ...
         */
    const isUserInThisClass = isClass.bicis.findIndex(
      (item) => item.userId == req.user._id.toString()
    );
    if (isUserInThisClass >= 0) {
      return res
        .status(409)
        .json({ message: "Ya estas registrado en esta horario de clases" });
    }

    /* 3.
            extra validadion que no deja asignar el mismo numero de bici dos veces
         */
    const isAssign = isClass.bicis.findIndex(
      (item) => item.noBici == req.body.noBici
    );
    if (isAssign >= 0) {
